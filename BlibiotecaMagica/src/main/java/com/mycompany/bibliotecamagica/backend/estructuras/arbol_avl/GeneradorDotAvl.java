...
public class GeneradorDotAvl {
    ...
    public String obtenerDot(NodoAvl<LibroBiblioteca> raiz){
    ...
    private void agregarDatosRecursivos(NodoAvl<LibroBiblioteca> nodo, StringBuilder datos) {
        String titulo = nodo.getLista().obtenerPrimero().getLibro().getSinGuiones();
        datos.append(NOMBRE_NODO).append(titulo);
        datos.append(" [label=\" ");
        if(!nodo.esNodoHoja()) datos.append("<f0> |");
        datos.append("Titulo: ").append(nodo.obtenerPrimero().getLibro().getTitulo()).append("\\n");
        ...
        if (nodo.getIzquierda() != null) {
            ...
            datos.append(NOMBRE_NODO).append(nodo.getIzquierda().obtenerPrimero().getLibro().getSinGuiones()).append(";\n");
        }
        if (nodo.getDerecha() != null) {
            agregarDatosRecursivos(nodo.getDerecha(), datos);
            datos.append(NOMBRE_NODO).append(titulo).append(":f1 -> "); 
            datos.append(NOMBRE_NODO).append(nodo.getDerecha().obtenerPrimero().getLibro().getSinGuiones()).append(";\n");
        }
    }
}
